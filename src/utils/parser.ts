 ...
 */
export function parseConversionInput(input: string): ParsedInput | ErrorDetails {
  ...
  return {
    ...
    context: 'Please use the format: "NUMBER UNIT to UNIT" or "NUMBER UNIT as UNIT". Examples: "5 meters to feet", "2.5 inches as millimeters", "1/2 foot to cm"',
    suggestions: [
 ...
 */
export function validateUnits(sourceUnit: string, targetUnit: string): ErrorDetails | null {
 ...
 */
export function parseAndValidateInput(input: string): ParsedInput | ErrorDetails {
  ...
  const validationError = validateUnits(parseResult.sourceUnit, parseResult.targetUnit);
  if (validationError) {
 ...
 */
export function parseMultilineInput(input: string): (ParsedInput | ErrorDetails)[] {
 ...
 */
export function isSuccessfulParse(result: ParsedInput | ErrorDetails): result is ParsedInput {
